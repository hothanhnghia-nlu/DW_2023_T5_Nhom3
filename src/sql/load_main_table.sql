                ...
                IF runningCount = 0 THEN 
                    ...
                    IF successfulCount = 0 THEN 
                        ...
                        BEGIN
                            ...
                            UPDATE control.log SET time = NOW(), status = 'failed' WHERE id = ( SELECT id FROM control.log WHERE process_id = 4 AND status = 'start' and time >= NOW() - INTERVAL 1 HOUR);
                            ...
                            GROUP BY
                                ...
                                -- 12.Insert temp_data_province , temp_data_weather, temp_data_main_table into
                                ...
                                BEGIN 
                                    ...
                                    UPDATE control.log SET time = NOW(), status = 'successful' WHERE id = ( SELECT id FROM control.log WHERE process_id = 4 AND status = 'start' and time >= NOW() - INTERVAL 1 HOUR );
                                    END IF;
                                END;
                        END IF;
                        END;
                    ELSE
                        -- 7.Insert Table log(control):time:now, process_id:4,status:failed 
                        INSERT INTO control.log (time, process_id, status)
                        VALUES (NOW(), 4, 'failed');
                    END IF;
                ELSE
                    -- 5.Insert Table log(control):time:now, process_id:4,status:failed 
                    INSERT INTOcontrol.log (time, process_id, status)
                    VALUES(NOW(), 4, 'failed');
                END IF;
        ELSE 
        -- 3.Insert Table log(control):time:now, process_id:4,status:failed 
            INSERT INTO control.log (time, process_id, status)
            VALUES(NOW(), 4, 'failed');
        END IF;
